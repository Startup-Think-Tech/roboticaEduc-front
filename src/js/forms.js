...
function validateInput(inputElement) {
    ...
    else if (inputElement.type === 'email' && !isValidEmail(inputElement.value.trim())) {
        isValid = false;
        message = 'Por favor, insira um e-mail válido.';
    }
    // 3. Validação de Estrelas (aplica-se ao campo hidden)
    else if (inputElement.id === 'estrelas' && inputElement.dataset.min && parseInt(inputElement.value) < parseInt(inputElement.dataset.min)) {
        isValid = false;
...
function initFormValidation(formId) {
    ...
    const inputElements = form.querySelectorAll('input[required], textarea[required], input[data-min]');
    ...
    form.addEventListener('submit', async function(event) {
        ...
        if (formIsValid) {
            ...
            if (!endpoint) {
                return displayStatusMessage('error', '❌ Configuração de endpoint não encontrada.');
            }
            ...
            try {
                ...
                if (response.ok) { // Status HTTP 200-299
                    ...
                    const errorData = await response.json().catch(() => ({ message: 'Resposta de erro sem JSON.' }));
                    
                    console.error('Erro de envio:', response.status, errorData);
                    displayStatusMessage('error', `❌ Erro (${response.status}): ${errorData.message || 'Falha ao processar a requisição no servidor.'}`);
                }

            } catch (error) {
                // Trata erros de rede
                console.error('Erro de rede ou fetch:', error);
                displayStatusMessage('error', '❌ Erro de conexão! Verifique se o servidor está ativo (CORS).');
            }

        } else {
            // Exibir mensagem de erro de validação de frontend
            displayStatusMessage('error', '❌ Erro! Por favor, preencha os campos obrigatórios/inválidos.');
        }
    });
}
